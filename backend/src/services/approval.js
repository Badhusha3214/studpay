const APPROVAL_GRADE_THRESHOLD = 5;
...
function needsJunkApproval(cls, categories) {
  const grade = gradeOf(cls);
  if (grade === null || grade > APPROVAL_GRADE_THRESHOLD) return false;
  return categories.includes('junk');
}

module.exports = { gradeOf, needsJunkApproval, APPROVAL_GRADE_THRESHOLD, APPROVAL_TIMEOUT_MS };
