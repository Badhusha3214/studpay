...
router.get('/balance', authMiddleware, (req, res) => {
  const student = db.prepare('SELECT id, name, email, class, balance FROM students WHERE id = ?').get(req.user.id);
...
router.post('/pay', authMiddleware, (req, res) => {
  ...
  const newBalance = student.balance - Number(amount);
  db.prepare('UPDATE students SET balance = ? WHERE id = ?').run(newBalance, req.user.id);
...
router.post('/pay-by-nfc', authMiddleware, adminMiddleware, (req, res) => {
  ...
  db.prepare('UPDATE students SET balance = ? WHERE id = ?').run(newBalance, card.id);
